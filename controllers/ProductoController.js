...
const agregar_imagen_galeria_admin = async function (req,res) {
    if(req.user){
        if(req.user.role == 'admin'){
            ...
            let newGaleria = {imagen: imagen_name, _id: data._id}
            let reg = await Producto.findById(id);
            reg.galeria.push(newGaleria);
            await reg.save();

            console.log(reg.galeria);
...
const eliminar_imagen_galeria_admin = async function (req,res) {
    if(req.user){
        if(req.user.role == 'admin'){
            ...
            let reg = await Producto.findByIdAndUpdate({_id:id},{$pull: {
...
const listar_productos_recomendados_publico = async function(req, res){
    ...
    let reg = await Producto.find({categoria: filtro}).sort({createdAt:-1}).limit(8);
    res.status(200).send({data:reg});
}
